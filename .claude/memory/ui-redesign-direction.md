...
- Keyboard model: `n` opens the composer at the next free slot; Esc closes it; drag with pointer. Blocks are also fully keyboard-operable (shipped 2026-06-01 in an `/impeccable audit`+`harden` pass): a movable block is focusable (`tabIndex=0`, accent `:focus-visible` ring), and arrow keys move (±15m), Shift+arrows resize, Enter/Space edits the title, Delete/Backspace removes, Escape reverts an uncommitted nudge. Saves are debounced (~450ms) so a burst of presses is one `rescheduleAction`. Lives in both `DayColumn.tsx` and `WeekColumns.tsx` (`onBlockKeyDown`/`commitKeyAdjust`); cross-day moves stay a pointer affordance.

**Deferred for a future pass (craft moves, not blockers):** ⌘K-style quick-add palette, an inline undo for delete (brief specified it; currently delete is immediate-and-final), shortcuts cheatsheet. Also open as a future quality pass: dark-theme `--text-faint` measures ~4.3:1 (marginally under WCAG AA for small text) and a few faint-at-reduced-opacity labels; left for an `/impeccable colorize` pass with in-browser verification rather than a blind token tweak. A composer-created block can't end exactly at midnight (24:00 normalizes to 12:00 am and fails end>start) — rare, and a real fix touches the server wall-clock→UTC path.
