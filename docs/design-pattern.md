# Kairos — Design Patterns (index)

> _"The right & opportune time to do a task."_

This is the **index** for the design patterns that give each slice of Kairos its structure. The detailed, per-concern pattern docs now live **next to the skill that owns them**, under each skill's `references/` folder — one [`design-pattern.md`](../.claude/skills/README.md) per skill, alongside that skill's `technology-stack.md` slice.

These docs are **prescriptive intent**, not yet-written code — the repository is still pre-implementation. They are meant to be folded into the builder skills so that when a skill scaffolds a slice, it builds on a known structure rather than improvising.
...
- **Complementary:** a handful of patterns Kairos genuinely needs are **not** in GoF — from Fowler's _Patterns of Enterprise Application Architecture_ (**PoEAA**), Buschmann et al.'s _Pattern-Oriented Software Architecture_ (**POSA**), and Microsoft's .NET architecture guidance. These are labelled **[non-GoF]** wherever they appear so the lineage stays honest.
...
## The per-skill pattern docs

Each lives at `.claude/skills/<skill>/references/design-pattern.md` (next to that skill's `technology-stack.md` slice):

| Skill | Pattern doc | Lead patterns |
|---|---|---|
| orchestration-builder | [design-pattern.md](../.claude/skills/orchestration-builder/references/design-pattern.md) | **Builder** (resource graph), **Bridge** (one contract, two runtimes), **Proxy** (reverse proxy) |
| backend-builder | [design-pattern.md](../.claude/skills/backend-builder/references/design-pattern.md) | **Layered/Clean Architecture** [non-GoF], **MVP** [non-GoF], **Facade** (application services), **State** (sync token), **Template Method** (worker) |
| database-builder | [design-pattern.md](../.claude/skills/database-builder/references/design-pattern.md) | **Data Mapper / Repository / Unit of Work** [non-GoF], **Builder** (model config), **Strategy** (value converters), **Factory Method** (`IDbContextFactory`) |
| mcp-builder | [design-pattern.md](../.claude/skills/mcp-builder/references/design-pattern.md) | **Adapter** (tools→services), **Command** (each tool), **Facade** (application services), **Remote Proxy** (`/mcp`) |
| frontend-builder | [design-pattern.md](../.claude/skills/frontend-builder/references/design-pattern.md) | **MVP / Page Controller** [non-GoF], **Composite** (partial tree), **Observer** (htmx/Alpine events), **Command** (keymap, drop) |
| observability-builder | [design-pattern.md](../.claude/skills/observability-builder/references/design-pattern.md) | **Facade** (`ServiceDefaults`), **Mediator** (the Collector), **Pipes & Filters** [non-GoF], **Observer** (instrumentation) |
| testing-builder | [design-pattern.md](../.claude/skills/testing-builder/references/design-pattern.md) | **Template Method** (fixtures), **Builder** (test data), **Shared-Fixture Singleton**, **Test Double** family [non-GoF] |
| devops-builder | [design-pattern.md](../.claude/skills/devops-builder/references/design-pattern.md) | **Chain of Responsibility** (CI gates), **Memento** (backups), **Feature Toggle / Strategy** [non-GoF] |

> Skill overview & build order: [.claude/skills/README.md](../.claude/skills/README.md). Canonical stack index: [technology-stack.md](technology-stack.md). Rationale & benchmarks: [research.md](research.md).
...
## What each per-skill file contains

Every `references/design-pattern.md` follows the same shape so it's easy to fold into a skill:
